...
class SpamFilterEvaluator:
    ...
    def find_false_positives(self, X_test, y_test, max_display=5):
        ...
        for i, (email, true_label, pred_label) in enumerate(zip(X_test, y_test, self.results['y_pred'])):
            if true_label == 0 and pred_label == 1:  # Légitime classé comme spam
                result = self.spam_filter.classify(email)
    ...
    def get_recommendations(self):
        ...
        # Recommandations pour faux positifs
        ...
        # Recommandations pour faux négatifs
        if r['recall'] < 0.95:
            recommendations.append(
    ...
    def analyze_phishing_detection(self, X_test, y_test):
     ...
     # Identifier les emails de phishing dans le test
     phishing_indices = []
     for i, email in enumerate(X_test):
        email_lower = email.lower()
        # Chercher des indicateurs de phishing
        phishing_indicators = [
     ...
     # Analyser les performances sur le phishing
     true_positives = 0
     false_negatives = 0
     ...
     for idx in phishing_indices:
        ...
        if is_phishing and predicted_spam:
            ...
            if false_negatives <= 3:  # Limiter l'affichage
